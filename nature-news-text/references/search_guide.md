# Nature News Text Retrieval Guide

This document describes how `nature-news-text` should retrieve, rank, cache, and reuse a shortlist of recent Nature news.

## Scope

Use this guide when `nature-news-text` needs to prepare a shortlist of recent Nature news summaries for the user.

The skill-level output behavior belongs in [SKILL.md](../SKILL.md). This file covers retrieval, shortlist generation, cache handling, reuse decisions, and the rule that full original English text is only provided after the user selects shortlist entries.
...
- optional later enrichment such as full text
...
- present the shortlist first, then ask the user to choose news numbers before providing any full text
